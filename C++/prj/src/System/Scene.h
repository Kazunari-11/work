...
public:
    ...
    using SignalsFilter  = sigslot::signal<>;
    ...
    public:
        ...
        virtual void LateUpdate([[maybe_unused]] float delta){};   //!< 通常更新の後更新処理
        ...
        SignalsUpdate& GetUpdateSignals(ProcTiming timing)
        {
            ...
            return signals_update_;   //< ワーニング対策
        }
        ...
        SignalsDefault& GetSignals(ProcTiming timing)
        {
            ...
            if(timing == ProcTiming::HDR)
                return signals_hdr_;
            if(timing == ProcTiming::Filter)
                return signals_filter_;
        ...
        //! Filterタイミングシグナル取得
        SignalsFilter& GetSignalsFilter() { return signals_filter_; }
        //----------------------------------------------------------------------
        //! @name Cereal セーブロード
        //----------------------------------------------------------------------
        //@{
    ...
    //! @brief シーン更新前処理
    //! @details 基本的にはUpdate集団より先にで行いたいものをこの層で処理します
    ...
    //! @brief Physics前処理
    //! @details OwnerObject移動によるコリジョンコンポーネント付随処理
    static void PrePhysics();

    //! @brief シーン更新後の処理
    //! @details Physicsおよびすべての当たり判定後に処理します
    ...
    //! @brief シーンGUI
    //! @details ※この処理はUpdate後に行う必要があります
    static void GUI();

...
void SetTexture(u32 slot, Texture* texture)
{
    if(texture) {
        // 使用するテクスチャを設定
        SetUseTextureToShader(slot, *texture);

        // [不具合対策] 直接RenderTargetを変更するため、DxLib内部のキャッシュと不整合が起きる。
        // 直接反映する
...
void CopyToRenderTarget(Texture* dest_render_target, Texture* source_texture, int shader_ps_handle)
{
    ...
    //----------------------------------------------------------
    {
        ...
        // [不具合対策] 直接RenderTargetを変更するため、DxLib内部のキャッシュと不整合が起きる。
        // 直接反映する
        ID3D11ShaderResourceView* srv = source_texture->srv();
        GetD3DDeviceContext()->PSSetShaderResources(0, 1, &srv);
#endif

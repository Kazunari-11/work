...
void Scene::Base::UnregisterAll()
{
    ...
    signals_filter_.disconnect_all();
}
...
void Scene::Base::setProc(ObjectPtr obj, SlotProc<T>& slot)
{
    ...
    if constexpr(std::is_same<T, float>{}) {
        ...
        if(slot.func_ == nullptr) {
            ...
            proc.SetProc(slot.GetFunc(), slot.GetTiming(), slot.GetPriority());
            auto p         = proc.GetFunc().get();
            auto ptr       = std::bind(&Callable<T>::Exec, p, std::placeholders::_1);
            proc.GetProc() = ptr;
            proc.connect_ = current_scene_->GetUpdateSignals(slot.GetTiming()).connect(proc.proc_, (int)proc.priority_);
        }
#endif
        ...
        if(slot.func_ == nullptr) {
            ...
            auto p         = proc.GetFunc().get();
            auto ptr       = std::bind(&Callable<T>::Exec, p);
            proc.GetProc() = ptr;
            proc.connect_  = current_scene_->GetSignals(slot.GetTiming()).connect(proc.proc_, (int)proc.priority_);
#if 0
			typedef void ( Callable::*Exec )();
			Exec ptr = &proc.GetFunc()->*Callable::Exec;
			//std::function<void()> f	= ptr;
			proc.connect_ = current_scene_->GetUpdateSignals( slot.GetTiming() ).connect( ptr, (int)proc.priority_ );
#endif
        }
#endif
    }
...
void Scene::checkSerialized(ObjectPtr obj)
{
    ...
    for(auto itr = obj->update_timings_.begin(); itr != obj->update_timings_.end();) {
        auto& proc = *itr;

        [[maybe_unused]] auto& p = proc.second;

        if(p.GetProc() == nullptr && p.GetFunc() == nullptr) {
            assert(
                0 &&
                "SetProcで追加した処理がSerializeされません。処理をProcFunc()で作成して登録するか、初期化する同じシーンを選択しInitSerialize()で初期化してください。ここは、「無視」することで進めますが、処理は復活しません");

            itr = obj->update_timings_.erase(itr);
            continue;
        }
        itr++;
    }
...
void Scene::checkSerialized(ComponentPtr comp)
{
    ...
    // 処理がnullptrのものがないかチェック
    for(auto itr = comp->update_timings_.begin(); itr != comp->update_timings_.end();) {
        auto&                  proc = *itr;
        [[maybe_unused]] auto& p    = proc.second;
        ...
        if(p.GetProc() == nullptr && p.GetFunc() == nullptr) {
            ...
            itr = comp->update_timings_.erase(itr);
            continue;
        }
        itr++;
    }
...
void Scene::Init()
{
    ...
    debug_scene_name = current_scene_->Name();
    if(next_scene_)
        debug_scene_name = next_scene_->Name();
}
...
void Scene::Update(float delta)
{
    if(current_scene_) {
        ...
        if(!scene_pause || scene_step) {
            current_scene_->LateUpdate(delta);
            scene_time += delta;
        }
...
void Scene::GUI()
{
    ...
    ImGui::Begin(name.c_str(), (bool*)nullptr, flags);
    {
        ...
        ImGui::BeginChild(ImGui::GetID((void*)Scene::GUI), ImVec2(xy.x - 40, xy.y * 0.5f), ImGuiWindowFlags_NoTitleBar);
        {
...
void Scene::CheckComponentCollisions()
{
    ...
    for(int obj_index = 0; obj_index < obj_num; obj_index++) {
        ...
        auto cols = obj->GetComponents<ComponentCollision>();
        {
            ...
            for(auto col_1 : cols) {
                // 相手オブジェクトの取得
                for(int other_index = 0; other_index < obj_num; other_index++) {

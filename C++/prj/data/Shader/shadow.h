cbuffer ShadowCb : register(b11)
{
	matrix mat_light_view_;   //!< シャドウ用ビュー行列
	matrix mat_light_proj_;   //!< シャドウ用投影配列
	matrix mat_light_view_proj_;   //!< シャドウ用ビュー×投影配列
};

Texture2D		ShadowTexture : register(t11);
...
// シャドウ遮蔽を計算
//---------------------------------------------------------------------------
float calcShadow(float3 worldPosition, float cameraDistance)
{
	float4	shadowPosition = mul(float4(worldPosition, 1.0), mat_light_view_proj_);
	float2	shadowUv = shadowPosition.xy * float2(0.5, -0.5) + 0.5;

    static const int SAMPLE_COUNT = 16;
    float shadow = 0.0;

    for (int i = 0; i < SAMPLE_COUNT; ++i) {
        float2 offset = poissonSamples[i] * 0.0005;
        float	shadowDepth = ShadowTexture.Sample(ShadowSampler, shadowUv + offset).r;

        static const float SHADOW_BIAS = 0.0002;
        if (shadowPosition.z < shadowDepth + SHADOW_BIAS) {
            shadow += 1.0 / SAMPLE_COUNT;
        }
    }

    // 距離でフェードアウト
    static const float SHADOW_DISTANCE = 400.0;	// シャドウ描画距離(m)
    static const float SHADOW_FADE_RANGE = 25.0;	// フェードさせる幅(m)

    shadow = lerp(1.0, shadow, saturate((SHADOW_DISTANCE - cameraDistance)*(1.0/SHADOW_FADE_RANGE) ));
